...
class TrackUserPage extends StatefulWidget {
  ...
  TrackUserPage({required this.ukey});

  @override
  _TrackUserPageState createState() => _TrackUserPageState();
}

class _TrackUserPageState extends State<TrackUserPage> {
    ...
    reference.onValue.listen((event) {
      if (event.snapshot.value != null) {
        ...
        if (userData is Map<Object?, Object?>) {
          ...
          double latitude = double.tryParse(userDataMap['latitude'].toString()) ?? 0.0;
          double longitude = double.tryParse(userDataMap['longitude'].toString()) ?? 0.0;
